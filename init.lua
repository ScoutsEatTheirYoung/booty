...
function commands.corpses(args)
    ...
    if #corpses == 0 then
        utils.print('No corpses found within radius.')
        return
    end
...
function commands.loot(args)
    ...
    if args[2] == 'help' then
        utils.print('booty loot - Loot items from nearest corpse')
        return
    ...
    elseif args[2] == 'filter' then
        ...
        if not filterName then
            utils.error('Please provide a name for the filter. Usage: /lua run booty loot filter [filterName]')
            return
        end
        local loadedFilter = filter.load(filterName)
        if not loadedFilter then
            utils.error('Could not load filter: ' .. filterName)
            return
        end
        local cfg = config.get().corpse
        loot.with_filter(loadedFilter, cfg.default_radius, cfg.default_zradius)
    end
end
...
function commands.config(args)
    ...
    elseif args[2] == 'reload' then
        config.reload()
        utils.success('Configuration reloaded')
    elseif args[2] == 'save' then
        config.save()
    elseif args[2] == 'get' then
        local path = args[3]
        if not path then
            utils.error('Usage: /lua run booty config get <path>')
            utils.info('Example: /lua run booty config get loot.max_distance')
            return
        end
        local value = config.value(path)
        utils.info(string.format('%s = %s', path, tostring(value)))
    elseif args[2] == 'set' then
        local path = args[3]
        local value = args[4]
        if not path or not value then
            utils.error('Usage: /lua run booty config set <path> <value>')
            utils.info('Example: /lua run booty config set loot.max_distance 20')
            return
        end
        ...
        utils.success(string.format('Set %s = %s', path, tostring(value)))
    elseif args[2] == 'path' then
        utils.info('Config file: ' .. config.get_user_config_path())
    else
        utils.print('Config commands:')
        utils.info('  dump/show  - Display current configuration')
        utils.info('  reload     - Reload configuration from disk')
        utils.info('  save       - Save current configuration to disk')
        utils.info('  get <path> - Get a config value (e.g., loot.max_distance)')
        utils.info('  set <path> <value> - Set and save a config value')
        utils.info('  path       - Show config file location')
    end
end
...
function commands.filter(args)
    if args[2] == 'create' then
        local filterName = args[3]
        if not filterName then
            utils.error('Please provide a name for the filter. Usage: /lua run booty filter create [filterName]')
            return
        end
    ...
    elseif args[2] == 'test' then
        local filterName = args[3]
        if not filterName then
            utils.error('Please provide a filter name. Usage: /lua run booty filter test [filterName]')
            return
        end
        local loadedFilter = filter.load(filterName)
        ...
        utils.print('Filter commands:')
        utils.info('  gui              - Open the filter editor GUI')
        utils.info('  create <name>    - Create a new filter skeleton')
        utils.info('  test <name>      - Test a filter against inventory')
    end
end
...
function commands.test(args)
    utils.info('Running booty test command ' .. utils.table_to_string(args))
    local cfg = config.get().corpse
    ...
    elseif args[2] == 'filter' then
        ...
        utils.info('Loaded filter: ' .. filterName)
...
if commands[command_name] then
    ...
    utils.error('Unknown command: ' .. tostring(command_name))
end
